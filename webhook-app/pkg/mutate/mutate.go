...
func Mutate(body []byte, verbose bool) ([]byte, error) {
	...
	if ar != nil {
		...
		for i := range pod.Spec.Containers {
			...
			log.Printf("Container %s, ImagePullPolicy: %s", containerName, imagePullPolicy)
			if imagePullPolicy == corev1.PullAlways {
				log.Print("Image pull policy set to pull always, moving to ifNotPresent")
		...
		if err != nil {
			log.Fatal("Error marshaling json object", err)
		}
